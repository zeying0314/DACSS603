...
This heat map explains the correlation values between specific variables for each cell.

Color scale: Positive correlations are usually colored blue and negative correlations are colored red. Strength: Darker colors indicate stronger correlations.

Positive correlation: Values close to 1 indicate a strong positive linear relationship.

Negative correlation: Values close to -1 indicate a strong negative linear relationship.

No correlation: Values close to 0 indicate a weak or no linear relationship.
...
For boxplot that the outliers represent data points worthy of attention.
...
From this graph, we can see that the regression assumptions is not violated, because this line is straight enough. Points are randomly distributed.
...
A random scatter of points around the horizontal axis, so it is homoscedasticity.

**Normal Q-Q (Quantile-Quantile) Plot:**

Points follow a straight line so it is non-normality.

**Scale-Location Plot:**

A horizontal line so it ishomoscedasticity.

**Cook's Distance Plot:**

It significantly impact the regression results because it has extreme point.

**Residuals vs. Leverage Plot:**

Thee is no points both high leverage and high residuals.

# Key Finding:

Our analysis reveals a subtle relationship between a health plan's restriction index (RI) and the average cost per prescription (COST). In addition, the interaction between RI and generic substitution (GS) percentage significantly affects drug costs. Contrary to our initial hypothesis, we observed that health plans with higher GS percentages showed a stronger positive correlation between RI and drug costs compared to health plans with lower GS percentages. This suggests that the interaction between formulary restrictions and generic substitution practices is a key determinant of drug costs.

Additionally, the study found that demographic factors, specifically average member age (AGE) and percentage of female members (F), contribute to differences in drug costs across health plans. Drug costs tend to be higher for health plans with older average members. In addition, plans with a higher percentage of female members show a significant correlation with increased drug costs.

For further research, we can study other explanatory Variables: RXPM, GS, RI, COPAY, MM to explore the relationship with the response Variable: COST, will get more outstanding findings I believe.\
